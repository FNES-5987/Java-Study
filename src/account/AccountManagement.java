...
public class AccountManagement {
    ...
    public AccountManagement(int maxSize) {
        accountList = new Account[maxSize]; // 계좌 목록 배열 생성
    ...
    public void addAccount(String accountNumber, double balance) {
        ...
        // 계좌번호와 초기 입금액을 입력 받고 계좌객체를 생성한 후 배열의 앞에서부터 빈 공간의 마지막에 추가한다.
        numAccounts++;// 단, 배열에 추가한 후 numAccounts를 증가시킨다.
    }

    public void showAccountList() {
        // **코드 작성
        // 계좌 목록을 출력
            for (int i = 0; i < numAccounts; i++) {      // 배열은 현제 계좌갯수만큼만 돌린다
                Account account = accountList[i];        // account에 i번째 배열의 내용 할당
                System.out.print("계좌번호: " + "[" + account.getAccountNumber() + "]" + ",");  // 배열 내용 중 계좌번호만 추출
                System.out.println("잔액: " + "[" + account.getBalance() + "]");               // 배열 내용 중 금액만 추출
                System.out.println();
            }
        // 예시 출력 형태
        // ------------------------------------
